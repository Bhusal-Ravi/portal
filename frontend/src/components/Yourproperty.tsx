...
function Yourproperty() {
    ...
    async function getUserProperties(nextOffset: number) {
        if (loading || !hasMore) return
        setLoading(true)
        try {
            const response = await fetch(`${base_url}/api/userproperty?offset=${nextOffset}&limit=${LIMIT}`, {
                method: "GET",
    ...
    return (
        <div className="max-w-6xl mx-auto px-6 pt-20 pb-20">
            ...
            <div className="mb-10">
                ...
                <h1 className="text-4xl font-semibold text-gray-900 tracking-tight">Your Properties</h1>
            </div>
                        ...
                        <div className="relative overflow-hidden h-52">
                            ...
                            <div className="absolute top-2 z-10 flex flex-row right-2  group">
                                <div className="bg-black opacity-70 hidden transform-all duration-200 group-hover:block cursor-pointer px-2 py-1 rounded-sm mr-2">
                                    <p className="text-white">Remove this property</p>
                                </div>
                                <button disabled={deleteLoading} onClick={()=>handleDelete(item.id)}><X className={`mr-1 ${item.favourite===true?"fill-red-500":''}  group-hover:scale-120 transform-all duration-200 cursor-pointer bg-white rounded-full h-7 w-7 p-1`}/></button>
                            </div>
