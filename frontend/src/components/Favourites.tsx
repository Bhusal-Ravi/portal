...
function Favourites() {
  ...
  async function getFavourites(nextOffset: number) {
    if (loading || !hasMore) return
    setLoading(true)
    try {
      const response = await fetch(`${base_url}/api/userfavourite?offset=${nextOffset}&limit=${LIMIT}`, {
        method: "GET",
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 pt-20 pb-20">
      ...
      <div className="mb-10">
        ...
        <h1 className="text-4xl font-semibold text-gray-900 tracking-tight">Favourites</h1>
      </div>
